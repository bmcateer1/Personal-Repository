...
def update(dt):
  ...
  threshold = 190 #Threshold value
  maxdata = max(displayData[-500:]) #max data point in the last 500 data points
  print("max output in last 500 points",maxdata)  #print statement for diagnostic purpose
  if (maxdata > threshold): #if the max value is over the threshold, then flexing occurs
    flexing=True
  else:
    flexing=False
