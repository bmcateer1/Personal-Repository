...
Pa1_off(1)=10; %Pressures with the pump off
Pa2_off(1)=10; 
Pp_off(1)=10;  
Pv_off(1)=10; 
...
figure %plots the contrasting results of when the double pump is off and when it is on with factored resistances
hold on
grid on
%plot(t,Pa1,'r')
%plot(t,Pa2, 'g')
...
figure %plots each different pressure for the single pump
subplot (2,2,1)
plot(t,Pp)
title('Pp Heart Pump')
xlabel('Time(in sec)')
...
%{
figure  %Plots  single pump on pressure vs double pump Pressure of Aorta
hold on
plot(t,Pa1, 'r')
plot(t,Pa1_dub, 'k')
plot(t,Pa2, 'g')
...
xlabel('Time (in sec)')
title('Pressures with Single pump vs Double pump')
legend('Single Pump Thoracic','Double PumpThoracic','Single Pump Abdominal','Double Pump Abdominal')
%}
end
